...
rem Add Windows DoT
rem netsh dns set global doh=no
rem netsh dns add global dot=yes ddr=no
rem netsh dns set global dot=yes ddr=no
rem reg add "HKLM\System\CurrentControlSet\Services\Dnscache\Parameters" /v "EnableAutoDoh" /t REG_DWORD /d "0" /f
...
rem reg add "HKLM\Software\Policies\Microsoft\Edge" /v "DnsOverHttpsTemplates" /t REG_SZ /d "https://dns.adguard-dns.com/dns-query?" /f
